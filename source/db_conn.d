...
struct DBConnection
{
    ...
    UserRet addUser(string email, string username, string password, string name = "", string desc = "")
    {
        ...
        auto Passwd = toPassword(password.dup);
        string HashPass = makeHash(Passwd).toString();
        password = parseHash(HashPass).toString();
    ...
    UserRet authUser(string email, string password)
    {
        ...
        auto Passwd = res["password"].toString().strip("\"");
        auto userSalt = parseHash(Passwd).salt;
        auto hashPasswd = makeHash(toPassword(password.dup), userSalt).toString();
